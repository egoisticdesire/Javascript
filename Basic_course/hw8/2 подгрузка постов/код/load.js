...
function init() {
    ...
    if (window.innerHeight === document.body.offsetHeight) {
        insertPosts();
        init(); // прокомментируйте, пожалуйста, почему
        // без этого вызова init() вроде бы работает 
        // так же, как с ним
    }
}
init();
...
document.addEventListener('scroll', function (event) {
    if (scrollCheck.getBoundingClientRect().top <= window.innerHeight) {
        ...
        insertPosts();
        // а для чего в шаблоне был указан event? он же не используется
